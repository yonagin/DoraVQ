...
def generate_samples_from_pixelcnn(pixelcnn_model, vqvae_model, labels, shape, device):
    ...
    with torch.no_grad():
        ...
        if hasattr(vqvae_model, 'vector_quantization'):
            ...
            # DoraVQ
            generated_codes_flat = generated_codes.view(-1)
            embeddings = vqvae_model.vq.embedding(generated_codes_flat)
            embeddings = embeddings.view(generated_codes.shape[0], -1, vqvae_model.vq.embedding_dim)
            embeddings = embeddings.permute(0, 2, 1).contiguous()
            embeddings = embeddings.view(generated_codes.shape[0], vqvae_model.vq.embedding_dim, *shape)
...
def main():
    ...
    parser.add_argument("--doravq_model_path", type=str, default=None)
    ...
    if args.doravq_model_path and os.path.exists(args.doravq_model_path):
        # Load DoraVQ
        doravq = DoraVQ(
